...
async function deleteRequest<T>(endpoint: string, params?: Record<string, any>): Promise<ApiResponse<T>> {
  try {
    const queryString = qs.stringify(params);
    const requestURL = queryString ? `${endpoint}?${queryString}` : endpoint;
    const response: AxiosResponse<T> = await api.delete(requestURL);
    return response.data as ApiResponse<T>;
  } catch (error) {
    console.error('DELETE request error: ' + endpoint, params, error);
    const newObj: ApiResponse<T> = {
      isSuccess: false,
      message: 'Có lỗi xảy ra, vui lòng thử lại',
      result: null,
